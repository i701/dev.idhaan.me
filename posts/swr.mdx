...
<CodeTitle title="Javascript" lang="js" />
```js // find all categories const allCategories = data.map((crime) => crime.category)
// convert all categories into a set const categorySet = new Set(allCategories) //
convert set back to an array const categories = [...categorySet]
